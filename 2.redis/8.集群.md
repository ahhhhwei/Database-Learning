...
<div>
  配置文件详见：<a href="Cluster" style="text-decoration: none;">Cluster</a>
</div>
...
- 有些指令，是能够操作多个 key 的，此时如果 key 是分散在不同的分片上，就会出现问题

  ![](./pic/8-key分散在不同分片上.png)

  > 可以使用 hash tag，哈希标签允许强制让多个 key 分配到同一个哈希槽
...
1. 节点 A 给节点 B 发送 ping 包，B 就会给 A 返回一个 pong 包。ping 和 pong 除了 message type 属性之外，其他部分都是一样的。这里包含了集群的配置信息（该节点的 id，该节点从属于哪个分片，是主节点还是从节点，从属于谁，持有哪些 slots 的位置...）。
2. 每个节点，每秒钟，都会给一些随机的节点发起 ping 包，而不是全发一遍。这样设定是为了避免在节点很多的时候，心跳包也非常多（比如有 9 个节点，如果全发，就是 9 * 8 有 72 组心跳了，而且这是按照 $N^2$ 这样的级别增长的）。
3. 当节点 A 给节点 B 发起 ping 包，B 不能如期回应的时候，此时 A 就会尝试重置和 B 的 tcp 连接，看能否连接成功。如果仍然连接失败，A 就会把 B 设为 `PFALL` 状态（相当于主观下线）。
4. A 判定 B 为 `PFALL` 之后，会通过 `Redis` 内置的 Gossip 协议，和其他节点进行沟通，向其他节点确认 B 的状态。（每个节点都会维护一个自己的 "下线列表"，由于视角不同，每个节点的下线列表也不一定相同）。
5. 此时 A 发现其他很多节点，也认为 B 为 `PFALL`，并且数目超过总集群个数的一半，那么 A 就会把 B 标记成 FAIL（相当于客观下线），并且把这个消息同步给其他节点（其他节点收到之后，也会把 B 标记成 FAIL）。

至此，B 就被判定为故障节点了。

> 以下三种情况会出现集群宕机（称为 fail 状态）：
>
> - **情况一**：某个分片，所有的主节点和从节点都挂了。
> - **情况二**：某个分片，主节点挂了，但是没有从节点。
> - **情况三**：超过半数的 master 节点都挂了。

#### 2.2 故障迁移

上述例子中，B 故障，并且 A 把 B 的 `FAIL` 消息告知集群中的其他节点。

- **如果 B 是从节点**，那么不需要进行故障迁移。
- **如果 B 是主节点**，那么会由 B 的从节点（比如 C 和 D）触发故障迁移。



所谓故障迁移，就是指把从节点提升成主节点，继续给整个 Redis 集群提供支持。

**故障迁移的具体流程：**

1. **从节点判断自身是否有参选资格**

   如果从节点和主节点已经太久没通信（此时认为从节点的数据和主节点差异太大），时间超过阈值，就失去竞选资格。

2. **具有资格的从节点（如 C 和 D）会先休眠一段时间**
...
3. **休眠结束后，开始拉票**
   ...
   - 比如 C 的休眠时间到了，C 会向其他所有集群中的节点请求投票。
   - **只有主节点才有投票资格**。
...
4. **投票与晋升**
   ...
   - 当 C 收到的票数超过主节点数的一半时，C 晋升为新的主节点。
...
5. **同步更新集群信息**
   ...
   C 将自己成为主节点的消息同步给集群中所有节点，大家更新自己保存的集群结构信息。

## 五、集群扩容

我们把 110 和 111 也加入到集群中，构成 3 主 6 从的集群，以 110 为 master，111 为 slave，数据分片扩展到 4 个。

> 集群的扩容操作，是一个成本较高，风险较大的操作
...
- 重新分配 slots
  ...
  ```shell
  redis-cli --cluster reshard 172.30.0.101:6379 # 填集群的任意节点地址
  ```
  ...
  - 多少个槽位要搬运？ 
    ...
    4096，因为我们现在有四个分片，所以是 16384 / 4

  - 哪个节点来接收？

    a2471fe1094b82d68c6b194f604bae38c081499c，110 那个节点的 id

  - 从哪些节点搬运？

    all，表示从其他所有节点都进行搬运

    > 输入 all 后会先给出搬运计划，输入 yes 后执行，不仅 slots 重新划分，对应的数据也会进行搬运，这是一个比较重量的操作

    > 也可以手动指定，从某一个或某几个节点来移动槽位，以 done 结尾
  ...
  > 在搬运过程中，对于那些不需要搬运的 key，访问是没有问题的，但是对于搬运中的 key，访问可能会出现问题。一般都是在没人访问的时候扩容

- 给新的主节点添加从节点

  ```shell
  redis-cli --cluster add-node 172.30.0.111:6379 172.30.0.101:6379 --cluster-slave --cluster-master-id a2471fe1094b82d68c6b194f604bae38c081499c
  ```
...
- 重新分配 slots
  ...
  然后交互，我们把 110 节点的 4096 个槽位号分为三份：1365 + 1365 + 1366，分别分给其他三个主节点，接收 id 填其他主节点的 id，源 id 填 110 节点的 id，分配三次。

  ![](./pic/8-缩容交互.png)

  三次分配完成后，110 节点已经不再持有槽位了
