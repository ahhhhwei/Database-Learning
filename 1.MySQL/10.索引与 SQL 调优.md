...
InnoDB 存储引擎使用 `B+` 树作为索引默认的数据结构来组织数据，为频繁查询的列建立索引可以有效地提升查询效率，那么如何利用索引编写出高效的 `SQL` 查询语句？以及如何分析某个查询是否用到了索引，当前的索引还有没有优化的空间？

#### 7.1.1 构建测试数据

我们首先使用 [sql脚本](sql\16.sql) 创建数据库表。

然后在数据库 `topic01` 中构建一个100万条数据的表，运行下面这个存储过程：
...
可以看到使用非索引字段查询同样一条记录的耗时是使用主键列的60倍左右。

![](./pic/10-非索引列查询.png)

#### 7.1.4 压测工具

使用 MySQL 自带的压测工具模拟多个客户端同时查询，观察测试结果：

```sh
mysqlslap -uroot -p123456 --concurrency=100 --iterations=100 --create-schema="topic01" --engine="innodb" --number-of-queries=100000 --query "select id, sn, name, mail, age, gender, class_id from topic01.index_demo where id = 1020000;"
```

- 可以看到查询时间随着访问量的增大还迅速增高

  ![](./pic/查询时间随着访问量的增大还迅速增高.png)

- 同时也可以使用 `show processlist` 显示当前正在运行的线程，以下是并发查询的线程列表

  ![](./pic/10-show-processlist.png)

#### 7.1.5 执行计划 EXPLAIN

在执行 `SELECT`，`DELETE`，`INSERT`，`REPLACE` 和 `UPDATE` 之前都可以用执行计划分析 `SQL` 语句的执行情况，以便优化 `SQL` 的查询

> 只是分析，不会真正执行 sql，最终返回分析结果
>
> 用法：EXPLAIN + 具体的 sql
...
![](./pic/10-主键explain.png)

##### 7.1.5.2 使用非索引列查询的执行计划

```sql
EXPLAIN select id, sn, name, mail, age, gender, class_id from index_demo where sn = '1020000'\G
```

![](./pic/10-非索引列explain.png)
...
SELECT 标识符，查询语句中 SELECT 的序号。如果整条查询语句中包含子查询或合并查询，则每个查询的编号依次递增

首先在 `navicat` 里把 `student` 复制为 `student1`

- 子查询：

  ```sql
  EXPLAIN SELECT * FROM student WHERE id = (SELECT id FROM student1 WHERE name = '宋江');
  ```
...
- 合并查询：
  ...
  ```sql
  explain select * from student union select * from student1;
  ```

  ![](./pic/10-合并查询.png)

###### 7.1.5.3.2 select_type 列

select 的类型，可以是下表中的任何一种

| select_type 值 | 说明                                   |
|---------------|----------------------------------------|
| SIMPLE        | 简单SELECT(不使用UNION或子查询)        |
| PRIMARY       | 外层查询                               |
| UNION         | UNION中的第二个及之后的SELECT语句      |
...
* 查询时数据行所在表的名称
* `<unionM,N>`: 表示合并查询用到的表，M 和 N是id列的值
* `<subqueryN>`: 表示子查询用到的表，N是id列的值

###### 7.1.5.3.6 partitions列
查询时数据行所在的分区，对于非分区表，值为 `NULL`。

###### 7.1.5.3.7 type列
连接类型，稍后重点讨论。

###### 7.1.5.3.8 possible_keys列
`possible_keys` 列显示SQL语句可能会用到的索引，如果该列为 `NULL` 则SQL语句没有可用的索引。这时可以检查 `WHERE` 子句中引用的列，并考虑是否需要在这些列上建立索引来提高查询的性能。

> 在一个表中可能创建很多个索引，这里出现的索引列并不一定是真实使用的索引

###### 7.1.5.3.9 key列
`key` 列表示SQL语句实际使用的索引，如果该列为 `NULL`，则SQL语句没有可用的索引。

> 实际使用到的索引

###### 7.1.5.3.10 key_len列
`key_len` 列表示SQL语句使用索引的字节长度，如果 `key` 列为 `NULL`，则 `key_len` 列也为 `NULL`。

> 和索引列使用的数据类型有关

###### 7.1.5.3.11 ref列
`ref` 列显示在查询过程中哪些列或常量与`key`列中指定的索引进行比较。如果值是 `func`，则使用的值是某个函数的结果，要查看具体使用了哪个函数，请使用 `EXPLAIN` 后面的 `SHOW WARNINGS` 来查看扩展的 `EXPLAIN` 输出。

> 记录的是与索引列比较的具体的值

###### 7.1.5.3.12 rows列
`rows` 列表示SQL语句执行查询需要**检查的行数**，对于InnoDB表这是一个估计值。**数据越小效率越高**。

###### 7.1.5.3.13 filtered列
- `filtered` 表示按表条件筛选的表中数据行的百分比。最大值是100，这意味着没有对数据行进行过过滤。从100开始递减的值表示过滤量在增加。
- 如果Rows列显示的预估行数为1000，filtered为50.00(50%)，Rows × filtered的结果是根据条件过滤后的行数，也就是符合查询条件的记录数为1000 × 50% = 500行
- 这个值**越大表示过滤的效率越高**
...
表中只有一行数据，不用任何扫描，性能极高，是const类型的特殊情况
